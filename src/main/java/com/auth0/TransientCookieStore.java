...
class TransientCookieStore {
    ...
    static void storeOriginData(HttpServletResponse response, String domain, SameSite sameSite, String issuer, String path, boolean isSecure) {
        store(response, StorageUtils.ORIGIN_DOMAIN_KEY, domain, sameSite, true, isSecure, path);
        store(response, StorageUtils.ORIGIN_ISSUER_KEY, issuer, sameSite, true, isSecure, path);
    }
